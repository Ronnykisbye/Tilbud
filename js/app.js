...
async function boot() {
  try {
    ...
    showMessage("Indtast vare og område – tryk derefter på Søg.");
  } catch (error) {
    console.error(error);
...
function bindEvents() {
  ...
  dom.sortSelect.addEventListener("change", () => {
    if (!state.lastSearch) {
      showMessage("Vælg først en vare og tryk på Søg. Så kan sorteringen bruges bagefter.");
      return;
    }
  ...
  document.querySelectorAll(".quick-products button").forEach(button => {
    button.addEventListener("click", () => {
      ...
      showMessage(`Varen "${button.dataset.product}" er valgt. Tryk på Søg for at finde tilbud.`);
...
function runSearch(locationName, product, radiusKm, saveHistory = true) {
  ...
  if (!location) {
    showMessage(`Jeg kan ikke finde området "${locationName}" endnu. Prøv fx Helsingør, 3000, Hillerød, København eller Brug min GPS.`);
    return;
  }
  ...
  // Hvis brugeren vælger en meget lille radius, prøver vi automatisk én gang med 7 km,
  // så appen ikke virker tom ved almindelige varesøgninger.
  let usedFallbackRadius = false;
  if (!offers.length && Number(radiusKm) < 7) {
    offers = findOffers({
      product,
      location,
      radiusKm: 7,
    ...
    usedFallbackRadius = offers.length > 0;
  }

  state.currentOffers = offers;
  state.lastSearch = { locationName: location.name, product, radiusKm: usedFallbackRadius ? 7 : Number(radiusKm) };

  if (saveHistory) addHistory(product);
  drawResults(offers, usedFallbackRadius);
}

function drawResults(offers, usedFallbackRadius = false) {
  ...
  const radiusText = usedFallbackRadius ? " Vi udvidede automatisk radius til 7 km, fordi den valgte radius ikke gav resultater." : "";
  showMessage(`Fundet ${offers.length} relevante tilbud. Viser de ${visible.length} bedste.${radiusText}`);
}
...
function renderSavedLists() {
  renderChips(dom.favoritesList, state.favorites, value => {
    ...
    showMessage(`Favoritten "${value}" er valgt. Tryk på Søg for at finde tilbud.`);
  });
  renderChips(dom.historyList, state.history, value => {
    dom.productInput.value = value;
    showMessage(`Søgningen "${value}" er hentet. Tryk på Søg for at køre den igen.`);
  });
}
...
function useGpsLocation() {
  ...
  navigator.geolocation.getCurrentPosition(
    position => {
      ...
      fillLocationSuggestions();
      showMessage("GPS-positionen er valgt. Tryk på Søg for at finde tilbud i nærheden.");
    },
